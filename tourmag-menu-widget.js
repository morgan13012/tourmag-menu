...
/* FORCER le mega menu au-dessus du contenu de la page */
    ...
    #tourmag-menu .nav-item.active .mega-menu {
        z-index: 2147483647 !important;
            ...
            /* FORCER toutes les images à 40px dans les newsletters */
            .mega-column img,
            .newsletter-list img,
            .mega-links img,
            .newsletter-icon img,
...
// Toggle menu mobile - fonction globale
    ...
    function initializeJS() {
        ...
        // Gestion du comportement selon le mode (Desktop hover/click vs Mobile)
        function setupBehavior() {
            ...
            if (isDesktop && !isMultiline) {
                // MODE HOVER (Desktop, menu sur 1 ligne)
                console.log('✅ MODE HOVER activé');
                document.body.classList.add('hover-mode');
                
            } else if (isDesktop && isMultiline) {
                // MODE CLIC (Desktop, menu sur plusieurs lignes)
                console.log('🖱️ MODE CLIC activé (menu multiligne)');
                ...
                navItems.forEach(item => {
                    ...
                    if (megaMenu && link) {
                        const clickHandler = function(e) {
                            ...
                            console.log('📱 Onglet:', link.textContent.trim());
                            
                            // Vérifier l'état AVANT de tout fermer
                            const wasActive = item.classList.contains('active');
                            console.log('État avant:', wasActive ? 'OUVERT' : 'FERMÉ');
                            
                            // Fermer TOUS les menus (y compris celui-ci)
                            navItems.forEach(otherItem => {
                                otherItem.classList.remove('active');
                            });
                            
                            // Réouvrir UNIQUEMENT si c'était fermé
                            if (!wasActive) {
                                item.classList.add('active');
                                console.log('Action: OUVERTURE');
                            } else {
                                console.log('Action: FERMETURE');
                            }
        ...
        // GESTION DES NEWSLETTERS (sous-menus déroulants)
        ...
        // GESTION DES MÉDIAS (sous-menus déroulants)
